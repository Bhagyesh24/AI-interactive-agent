...
def record_audio_with_silence_detection():
    ...
    # Pre-speech buffer: keep the last ~0.5s of audio before voice detected
    # so the beginning of words isn't clipped.
    pre_buffer_chunks = int(0.5 / CHUNK_DURATION)
    pre_buffer = []
    ...
    with stream:
        for chunk_index in range(max_chunks):
            ...
            if not recording_started and energy > ENERGY_THRESHOLD:
                ...
                # Prepend pre-buffer so we capture the start of speech
...
def voice_pipeline_loop():
    ...
    while True:
        try:
            ...
            # Show user's transcribed question immediately in the browser
            ui_queue.put({"state": "thinking", "user_text": transcribed, "ai_text": "Thinking..."})
